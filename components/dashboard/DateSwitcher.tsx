...
export default function DateSwitcher({ initialDate }: DateSwitcherProps) {
  ...
  if (!mounted) {
    return (
      <div className="h-10 w-48 bg-zinc-900 border border-slate-800 rounded-2xl shrink-0" />
    );
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="flex items-center rounded-2xl bg-card border border-border">
        {/* Prev Day Button */}
        <button
          onClick={handlePrevDay}
          disabled={isPending}
          className="p-2 rounded-xl text-muted-foreground hover:text-foreground hover:bg-muted disabled:opacity-50"
        ...
        <button
          ...
          disabled={isPending}
          className="flex items-center gap-2 px-3 sm:px-4 py-1.5 rounded-xl hover:bg-muted text-xs sm:text-sm font-semibold text-foreground"
        >
      ...
      {mounted && !isToday && (
        <button
          onClick={handleToday}
          disabled={isPending}
          className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-primary/10 border border-primary/20 hover:bg-primary/20 text-primary text-xs font-semibold"
        >
          <RotateCcw size={14} />
          <span>Today</span>
        </button>
