...
export function FilterCard({ className, children, ...props }: FilterCardProps) {
  return (
    <div
      className={cn(
        "p-5 sm:p-6 rounded-3xl border border-border bg-card text-card-foreground space-y-4",
        className,
      )}
      {...props}
    >
    ...
    return (
      <div className="relative w-full">
        ...
        <input
          ref={ref}
          className={cn(
            "h-11 w-full rounded-xl border border-border bg-background px-3.5 py-2 text-sm text-foreground placeholder:text-muted-foreground outline-none focus:border-primary/40",
            icon && "pl-10",
            className,
          )}
          {...props}
  ...
  return (
    <div className="relative w-full">
      ...
      <select
        ref={ref}
        className={cn(
          "h-11 w-full appearance-none rounded-xl border border-border bg-background px-3.5 py-2 pr-10 text-sm text-foreground cursor-pointer outline-none focus:border-primary/40",
          icon && "pl-10",
          className,
        )}
        {...props}
      ...
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            className={cn(
              "px-4 py-1.5 rounded-lg text-[10px] sm:text-xs font-bold uppercase tracking-wider h-9 flex items-center justify-center cursor-pointer",
              isActive
                ? opt.activeColor ||
...
export function FilterButton({
  ...
  return (
    <button
      className={cn(
        "h-11 px-5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-pointer select-none",
        variant === "primary" &&
          "bg-primary/10 hover:bg-primary/20 border border-primary/30 hover:border-primary/40 text-primary",
        variant === "secondary" &&
          "bg-card border border-border hover:bg-muted text-foreground",
        variant === "danger" &&
          "bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/25 hover:border-rose-500/35 text-rose-400",
        className,
      )}
      {...props}
    >
