...
export async function addDebtPaymentAction(data: {
  ...
  try {
    ...
    if (paymentAmount > currentRemaining) {
      throw new Error(`Payment cannot exceed remaining debt balance of ₦${currentRemaining.toLocaleString('en-NG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`);
    }
    ...
    const formattedAmount = `₦${paymentAmount.toLocaleString('en-NG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    const extraDesc = description?.trim() ? ` ${description.trim()}` : '';
    const transDesc = debt.type === 'owed_by_me'
      ? `Paid ${formattedAmount} towards debt to ${debt.person}.${extraDesc}`
      : `Received ${formattedAmount} repayment from ${debt.person}.${extraDesc}`;
